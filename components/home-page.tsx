...
export function HomePage() {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-7xl flex-col px-4 py-6 md:px-6 md:py-8">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
          <div>
            ...
            <h1 className="mt-5 max-w-4xl text-4xl font-semibold tracking-tight text-ink md:text-6xl">
              Mullet Guide
              <span className="mt-2 block text-xl font-medium text-muted md:text-2xl">
          ...
          <div className="flex flex-col items-start gap-4 lg:items-end">
            <ConnectButton />
            <div className="rounded-[24px] border border-border bg-white/75 px-4 py-3 text-sm text-muted">
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="panel p-6 md:p-7">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-2xl font-semibold">描述你的收益目标</h2>
            </div>
          ...
          <textarea
            ...
            rows={4}
            className="mt-6 w-full rounded-[28px] border border-border bg-white/80 px-5 py-4 text-base text-ink outline-none transition focus:border-accent"
        ...
        <div className="panel p-6 md:p-7">
          ...
          <h2 className="mt-3 text-2xl font-semibold">当前生成的界面 Schema</h2>
          <p className="mt-3 text-sm leading-6 text-muted">
            这个面板用于演示 Agent 并不直接输出静态页面，而是输出 block schema，再由前端 renderer 映射到组件。
          </p>
          <pre className="mt-5 max-h-[380px] overflow-auto rounded-[28px] border border-border bg-slate-950 p-4 text-xs leading-6 text-slate-100">
